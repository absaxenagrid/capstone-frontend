...
export const ApplicationsPage = () => {
  const [apps] = useState<Application[]>(getAppliedJobs);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <StatCard label="Applied" value={apps.length} sub="+2 this week" icon={<Briefcase className="w-4 h-4 text-blue-600" />} color="bg-blue-50" />
        <StatCard label="Interviewing" value={byStatus('Technical Interview')} sub="Next: Tech Interview" icon={<Users className="w-4 h-4 text-indigo-600" />} color="bg-indigo-50" />
        <StatCard label="Offers" value={byStatus('Offer')} sub="Expiring in 48h" icon={<Star className="w-4 h-4 text-green-600" />} color="bg-green-50" />
        <StatCard label="Archived" value={byStatus('Rejected')} sub="Past 12 months" icon={<XCircle className="w-4 h-4 text-slate-500" />} color="bg-slate-100" />
      </div>
                ...
                {apps.map((app) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-xs text-slate-500 hidden md:table-cell">
                      {app.nextStep ?? 'Awaiting Feedback'}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="card p-5">
          {selected ? (
            <>
              ...
              <button onClick={() => navigate(`/jobs/${selected.jobSlug}`)} className="mt-4 flex items-center gap-1.5 text-xs text-primary-600 hover:text-primary-700">
